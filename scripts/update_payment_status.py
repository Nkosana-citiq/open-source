...
def update_payments(session, applicant=None):
    ...
    if last_payment:
        if relativedelta(NOW, last_payment.date).months > 2 or relativedelta(NOW, last_payment.date).months == 2 and NOW.month > last_payment.date.month:
            set_state(session, 'lapsed', applicant.id)
        elif relativedelta(NOW, last_payment.date).months > 1 or relativedelta(NOW, last_payment.date).months == 1 and NOW.month > last_payment.date.month:
            set_state(session, 'skipped', applicant.id)
        elif relativedelta(NOW, last_payment.date).months > 0 or relativedelta(NOW, last_payment.date).months == 0 and NOW.month > last_payment.date.month:
            set_state(session, 'unpaid', applicant.id)
        else:
            set_state(session, 'paid', applicant.id)
    else:
        if relativedelta(NOW, applicant.date).months > 2 or relativedelta(NOW, applicant.date).months == 2 and NOW.month > applicant.date.month:
            set_state(session, 'lapsed', applicant.id)
